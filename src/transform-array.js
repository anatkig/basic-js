...
module.exports = function transform(arr) {
 ...
 for(let i=0;i<arr2.length;i++){
  console.log(Array.isArray(arr2[i])+"prov")
   ...
   if(arr2[i][y]==="--discard-prev"){ if(y-1===-1)arr2[i].splice(y,1)  else arr2[y].splice(i-1,2)};
   if(arr2[i][y]==="--double-next"){if(y+1===arr2[i].length)arr2[i].splice(y,1)  else arr2[i].splice(y,1,arr2[y+1])};
   if(arr2[i][y]==="--double-prev"){if(y-1===-1)arr2[i].splice(y,1)  arr2[i].splice(y,1,arr2[y-1])};
